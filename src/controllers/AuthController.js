...
exports.singup = async (req, res) => {
    try {
        ...
        DB.query('select user_id from users_auth where user_email = ? ', [email], async (err, data) => {
            ...
            const userid = uuidv4()
            const hashpassword = await HashPassword(password,10)
            DB.query(
...
exports.login = async(req,res)=>{
    try{
        ...
        DB.query('select *from users_auth where user_email = ? ', [email], async (err, data) => {
            if (err) {
            ...
            const token = jwt.sign(
                {id:user.id},jwtconfig.secret,{expiresIn:jwtconfig.expiresIn}
            )
            res.json({token})
        })
    }catch(err){
